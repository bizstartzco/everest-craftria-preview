...
(function () {
  ...
  // --- Testimonial reel ----------------------------------------------------
  // The row drifts on its own, so it must be stoppable: WCAG 2.2.2 asks for a
  // way to pause anything that moves for more than five seconds. Hover and
  // keyboard focus pause it in CSS; this is the explicit control.
  var reel = $('[data-reel-track]');
  var reelToggle = $('[data-reel-toggle]');
  if (reel && reelToggle) {
    var reelLabel = $('[data-reel-label]', reelToggle);
